...
div このページはZZZ
  div
    NuxtLink(:to='switchLocalePath("ja")') 日本語
    NuxtLink(:to='switchLocalePath("en")') 英語
    NuxtLink(:to='switchLocalePath("vi")') ベトナム

  //- div(v-if="searchProductsQueryLoading") ロード中
  //- div(v-else) {{ searchProductsQueryResult.Products }}
  ...
  n-table(:single-line='false')
    n-thead
      n-tr
        ...
        n-th 価格
    n-tbody
      //- n-tr(v-if="searchProductsQueryLoading"): td ロード中
      //- n-tr(v-else): td ロード終わり
      template(v-for='item in searchProductsQueryResult.products', :key='item.id')
        n-tr
